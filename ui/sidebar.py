...
def sidebar() -> Tuple[str, List[dict], Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    ...
    with st.sidebar.expander("Quick Controls", expanded=True):
        c1, c2 = st.columns(2)
        engine_on = c1.toggle("Engine", value=est["running"])
        ...
        risk_pct = c4.number_input("Risk %/trade", min_value=0.001, max_value=0.1,
                                   step=0.001, format="%.3f", value=float(s["risk"].get("risk_pct", 0.01)))
        max_pos = c5.number_input("Max positions", min_value=1, max_value=50, value=int(s["risk"].get("max_positions", 6)))
        ...
        _chip("Running" if est["running"] else "Stopped", "ok" if est["running"] else "idle")
        _chip(f"Active {est['active_symbols']}", "ok" if est["active_symbols"] > 0 else "idle")
        last_local = est["last_tick_utc"]
        if last_local:
            _chip("Last tick ✓", "ok")
        else:
            _chip("No tick", "idle")
    ...
    opts["big_boss"]["enabled"] = bool(big_boss_on)
    opts["min_conf"] = conf_min
    risk["cooldown_min"] = int(cooldown)
    risk["rr"] = float(rr)
    risk["risk_pct"] = float(risk_pct)
